...
int main(int argc, char *argv[]) {
  if (argc != 5) {
    fprintf(stderr, "usage: writer D F N outputfile\n");
    fprintf(stderr, "  where D = delay in microseconds between writes\n");
  ...
  const int microseconds_between_writes = atoi(argv[1]);
  const long writes_between_flushes = atol(argv[2]);
  const long num_writes = atol(argv[3]);
